...
SELECT -1, 'tester', '{noop}test123$', 'tester@test.com', 'foo', DATEADD('MONTH', 3, CURRENT_TIMESTAMP) FROM DUAL UNION ALL
SELECT -2, 'leader', '{noop}lead123$', 'leader@test.com', 'bar', DATEADD('MONTH', 3, CURRENT_TIMESTAMP) FROM DUAL
...
DROP TABLE IF EXISTS MAP_USER_TEAM CASCADE;
CREATE TABLE IF NOT EXISTS MAP_USER_TEAM
(
    ID      BIGINT PRIMARY KEY,
    USER_ID BIGINT,
    TEAM_ID BIGINT,
...
COMMENT ON COLUMN AUTHORITY.DSC IS '권한 설명';

INSERT INTO AUTHORITY (ID, PID, ROLE, DSC) SELECT * FROM (
...
SELECT -11, -10, 'UNTOUCHABLES', '불가촉천민' FROM DUAL
) WHERE NOT EXISTS(SELECT * FROM AUTHORITY);


DROP TABLE IF EXISTS MAP_USER_AUTHORITY CASCADE;
CREATE TABLE IF NOT EXISTS MAP_USER_AUTHORITY
(
    ID           BIGINT PRIMARY KEY,
...
COMMENT ON TABLE MAP_USER_AUTHORITY IS '사용자-권한 매핑';

INSERT INTO MAP_USER_AUTHORITY (ID, USER_ID, AUTHORITY_ID) SELECT * FROM (
...
) WHERE NOT EXISTS (SELECT * FROM MAP_USER_AUTHORITY);

DROP TABLE IF EXISTS MAP_TEAM_AUTHORITY CASCADE;
CREATE TABLE IF NOT EXISTS MAP_TEAM_AUTHORITY
(
    ID           BIGINT PRIMARY KEY,
...
COMMENT ON TABLE MAP_TEAM_AUTHORITY IS '부서-권한 매핑';


INSERT INTO MAP_TEAM_AUTHORITY (ID, AUTHORITY_ID, TEAM_ID) SELECT * FROM (
...
) WHERE NOT EXISTS( SELECT * FROM MAP_TEAM_AUTHORITY );
...
COMMENT ON COLUMN MENU.PROPS IS '속성';
...
DROP TABLE IF EXISTS MAP_MENU_AUTHORITY CASCADE;
CREATE TABLE IF NOT EXISTS MAP_MENU_AUTHORITY
(
    ID           BIGINT PRIMARY KEY,
    AUTHORITY_ID BIGINT NOT NULL,
...
COMMENT ON TABLE MAP_MENU_AUTHORITY IS '메뉴-권한 매핑';

INSERT INTO MAP_MENU_AUTHORITY (ID, AUTHORITY_ID, MENU_ID) SELECT * FROM (
SELECT -1 AS A, -1 AS B, -2 AS C FROM DUAL UNION ALL
SELECT -2 AS A, -2 AS B, -2 AS C FROM DUAL UNION ALL
SELECT -3 AS A, -3 AS B, -2 AS C FROM DUAL
) WHERE NOT EXISTS( SELECT * FROM MAP_MENU_AUTHORITY );
...
ALTER TABLE MAP_USER_AUTHORITY
	ADD CONSTRAINT MAP_USER_AUTHORITY_FK1
		FOREIGN KEY (USER_ID) REFERENCES USR;

ALTER TABLE MAP_USER_AUTHORITY
	ADD CONSTRAINT MAP_USER_AUTHORITY_FK2
		FOREIGN KEY (AUTHORITY_ID) REFERENCES AUTHORITY;

ALTER TABLE MAP_TEAM_AUTHORITY
	ADD CONSTRAINT MAP_TEAM_AUTHORITY_FK1
		FOREIGN KEY (TEAM_ID) REFERENCES TEAM;

ALTER TABLE MAP_TEAM_AUTHORITY
	ADD CONSTRAINT MAP_TEAM_AUTHORITY_FK2
		FOREIGN KEY (AUTHORITY_ID) REFERENCES AUTHORITY;

ALTER TABLE MAP_MENU_AUTHORITY
	ADD CONSTRAINT MAP_MENU_AUTHORITY_FK1
		FOREIGN KEY (MENU_ID) REFERENCES MENU;

ALTER TABLE MAP_MENU_AUTHORITY
	ADD CONSTRAINT MAP_MENU_AUTHORITY_FK2
		FOREIGN KEY (AUTHORITY_ID) REFERENCES AUTHORITY;
