...
COMMENT ON COLUMN _FILE.TYPE IS 'Mime-Type(Content-Type)';
...
-- Article Entity
--
DROP TABLE IF EXISTS _ARTICLE_FILE CASCADE;
CREATE TABLE IF NOT EXISTS _ARTICLE_FILE
(
    ARTICLE_ID BIGINT,
    FILE_ID    VARCHAR(128),
    MAPPED  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (ARTICLE_ID, FILE_ID)
);
COMMENT ON TABLE _ARTICLE_FILE IS '게시글 첨부파일';
...
COMMENT ON COLUMN _ARTICLE.BUNCH IS '답글용 묶음 ID';
COMMENT ON COLUMN _ARTICLE.DEPTH IS '답글 깊이';
COMMENT ON COLUMN _ARTICLE.SEQUEL IS '답글 정렬 순서';
...
COMMENT ON COLUMN _ARTICLE_COMMENT.BUNCH IS '묶음 ID';
COMMENT ON COLUMN _ARTICLE_COMMENT.DEPTH IS '답글 깊이';
COMMENT ON COLUMN _ARTICLE_COMMENT.SEQUEL IS '정렬 순서';
...
ALTER TABLE _ARTICLE_FILE
    ADD CONSTRAINT _ARTICLE_FILE_FK1
        FOREIGN KEY (ARTICLE_ID) REFERENCES _ARTICLE;

ALTER TABLE _ARTICLE_FILE
    ADD CONSTRAINT _ARTICLE_FILE_FK2
